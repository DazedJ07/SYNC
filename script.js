...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateGreetings() {
        ...
        // Animate each character with GSAP stagger
    ...
    function renderCharts() {
        ...
        const absentCount = allEmployees.filter(e => e.status === 'Absent' || e.status === 'Late').length;
        ...
        if (deptCtx) {
            ...
            window.deptChart = new Chart(deptCtx.getContext('2d'), { 
                type: 'bar', 
                data: { 
                    labels: ['SOFTDEV', '3D DESIGN', 'Unassigned'], 
                    datasets: [{ 
                        label: 'Headcount', 
                        data: [
                            allEmployees.filter(e => e.department === 'SOFTDEV').length,
                            allEmployees.filter(e => e.department === '3D DESIGN TEAM').length,
                            allEmployees.filter(e => e.department !== 'SOFTDEV' && e.department !== '3D DESIGN TEAM').length
                        ], 
    ...
    // Allows admin to download attendance report as CSV file
    // Format: Employee ID, Full Name, Department, Batch, Team, Status
    
    const expBtn = document.getElementById('export-csv-btn');
    if(expBtn) {
        expBtn.addEventListener('click', () => {
            // Create CSV header
            let csvContent = "data:text/csv;charset=utf-8,Employee ID,Full Name,Department,Batch,Team,Status\n";
            
            // Add data rows
            allEmployees.forEach(emp => { 
                csvContent += `${emp.emp_id},${emp.full_name},${emp.department},${emp.batch || 'Batch 1'},${emp.team || 'Unassigned'},${emp.status || 'Absent'}\n`; 
            });
    ...
    function generateStudentCalendar() {
        ...
        for(let day = 1; day <= daysInMonth; day++) {
            ...
            if (isCurrentMonth && day === today.getDate() && currentUser.status && currentUser.status !== 'Absent') {
                classes = "cal-day colored ";
                if(currentUser.status === 'Present') classes += "bg-success";
                else if(currentUser.status === 'Late') classes += "bg-warning";
                else if(currentUser.status === 'Excused') classes += "bg-purple";
            }
    ...
    const BENTO_GLOW_COLOR = '139, 115, 85';
